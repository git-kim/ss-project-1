...
def main() -> None:
    ...
    config = EmergencyInfoApiConfig(
        api_relative_url="getEmgPatientTransferInfo",
        output_dir=Path(PROJECT_DIRECTORY_PATH / "data/raw/구급환자이송정보"),
        year_month_key="stmtYm",
    )

    client = EmergencyInfoApiClient(config)
